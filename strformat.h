...
private:
	...
	bool advance_();
	void clear();
	Part *format_f_(double value, bool trim_fractional_zeros);
	...
	Part *format_(char c, int hint);
	Part *format_(double value, int hint);
	Part *format_(int32_t value, int hint);
	Part *format_(uint32_t value, int hint);
	Part *format_(int64_t value, int hint);
	Part *format_(uint64_t value, int hint);
	Part *format_(char const *value, int hint);
	Part *format_(std::string const &value, int hint);
	strformat &format(std::function<Part *(int)> callback, int width, int precision);
public:
	strformat(std::string const &text);
	~strformat();
	strformat &reset();
	void destroy();
	strformat &add(std::string const &s);
	strformat &add(char const *s);
	template <typename T> strformat &a(T const &value, int width = -1, int precision = -1)
	{
		return format([&](int hint){ return format_(value, hint); }, width, precision);
	}
	...
	{
		return format([&](int hint){ return format_o32(value, hint); }, width, precision);
	}
	strformat &lo(int64_t value, int width = -1, int precision = -1)
	{
		return format([&](int hint){ return format_o64(value, hint); }, width, precision);
	}
	strformat &x(int32_t value, int width = -1, int precision = -1)
	{
		return format([&](int hint){ return format_x32(value, hint); }, width, precision);
	}
	strformat &lx(int64_t value, int width = -1, int precision = -1)
	{
		return format([&](int hint){ return format_x64(value, hint); }, width, precision);
	}
